...
def generate_article_content(keyword, vectorstore):
    ...
    Aim for an article length of about 800-1000 words.
    ...
    article_content = chain.run(keyword=keyword)
    return article_content

def generate_html5_page(keyword, article_content):
    html_template = f"""
    <!DOCTYPE html>
    <html lang="en">
    <head>
...
def main():
    ...
    # Keyword selection
    keywords = ["Description", "Topics", "Summary", "Advice"]
    selected_keyword = st.selectbox("Select a topic for the article:", keywords)

    # Process PDFs and generate HTML5 article
    if uploaded_files and selected_keyword:
        if st.button("Generate HTML5 Article"):
            with st.spinner("Processing PDFs and generating HTML5 article..."):
                ...
                article_content = generate_article_content(selected_keyword, vectorstore)
                html5_page = generate_html5_page(selected_keyword, article_content)
                
                st.subheader(f"Generated HTML5 Article: {selected_keyword}")
                ...
                st.download_button(
                    ...
                    data=html5_page,
                    file_name=f"{selected_keyword.replace(' ', '_')}_article.html",
                    mime="text/html"
                )
